 ...
 */
package com.alibaba.lindorm.contest.impl.store;

import com.alibaba.lindorm.contest.impl.store.bo.BytesRef;
import com.alibaba.lindorm.contest.impl.store.common.Accountable;
import com.alibaba.lindorm.contest.impl.store.common.RamUsageEstimator;
import com.alibaba.lindorm.contest.impl.store.utils.BitUtil;
import com.alibaba.lindorm.contest.impl.store.utils.UnicodeUtil;
...
public final class ByteBuffersDataOutput extends DataOutput implements Accountable {
    ...
    public static final long NEGATIVE_ZERO_DOUBLE = Double.doubleToLongBits(-0d);
    ...
    public void writeZDouble(double d) {
        int intVal = (int) d;
        ...
        if (d == intVal && intVal >= -1 && intVal <= 0x7C && doubleBits != NEGATIVE_ZERO_DOUBLE) {
            ...
            this.writeByte((byte) (0x80 | (intVal + 1)));
            return;
        } else if (d == (float) d) {
            // d has an accurate float representation: 5 bytes
            this.writeByte((byte) 0xFE);
            this.writeInt(Float.floatToIntBits((float) d));
        } else if ((doubleBits >>> 63) == 0) {
            // other positive doubles: 8 bytes
            this.writeByte((byte) (doubleBits >> 56));
            this.writeInt((int) (doubleBits >>> 24));
            this.writeShort((short) (doubleBits >>> 8));
            this.writeByte((byte) (doubleBits));
        } else {
    ...
    public static ByteBuffersDataOutput newResettableInstance() {
        ByteBufferRecycler reuser =
            new ByteBufferRecycler(ByteBuffersDataOutput.ALLOCATE_BB_ON_HEAP);
